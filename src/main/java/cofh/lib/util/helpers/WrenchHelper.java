...
public final class WrenchHelper {
	...
	public static boolean isHoldingUsableWrench(EntityPlayer player, BlockPos pos) {
		...
		if (equipped instanceof IToolHammer) {
			return ((IToolHammer) equipped).isUsable(player.inventory.getCurrentItem(), player, pos);
	...
	public static void usedWrench(EntityPlayer player, BlockPos pos) {
		...
		Item equipped = player.inventory.getCurrentItem() != null ? player.inventory.getCurrentItem().getItem() : null;
		if (equipped instanceof IToolHammer) {
			((IToolHammer) equipped).toolUsed(player.inventory.getCurrentItem(), player, pos);
		} else if (bcWrenchExists) {
